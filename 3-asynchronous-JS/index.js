...
const ambilGambarAnjing = async () => {
  try {
    ...
    const semuaResPro = await Promise.all([res1Pro, res2Pro, res3Pro]);
    const gambar = semuaResPro.map((el) => el.body.message);

    console.log(gambar);

    await writeFilePromise('./starter/dog-img.txt', gambar.join('\n'));
